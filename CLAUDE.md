...
**内点MSY非存在の構造定理（Phase 14, 2026-07-29）**: Phase 13 で経験的に観測された「上限張り付き・内部最大なし」は、グリッド探索の結果ではなく**解析的に導ける必然**であることを確認した。$A$ の対角0（種内競争なし）の一般化LVでは平衡が2つの2×2線形系に分解でき、`L·y_eq = r_x − f_x` / `(C∘L)·x_eq = r_y + f_y` より **x_eq は f_x に、y_eq は f_y に一切依存しない**（＝**Volterra の原理**: 自種の漁獲が自種の平衡量を変えない）。ゆえに平衡収量 $Y_{eq}(f)=\sum_i f_i B_{eq,i}(f)$ は **f について多重線形**で、箱 $[0,f_{max}]^4$ 上の最大は必ず頂点で達成される → **内点最大（MSY）は原理的に非存在**、報告値は探索上限 $f_{max}$ の人工物。数値検証（自由推定12変数, 既存 `sustainability.build_A_rho` を使用・コード無改変）: ∂x_eq/∂f_x = ∂y_eq/∂f_y = **厳密に0**（両レジーム）、各 f_j 方向の2階差分 = 4.3e−12(NLM)/1.4e−12(LM)＝機械精度、頂点16点の最大が内部ランダム20万点を上回る。**留保**: 厳密な多重線形性は平衡収量の性質であり、実際の計算は有限T の時間平均収量＋持続性制約なので頂点性は厳密には保証されない（数値実験は同じ構造を示す）。**この結論は推定値に依存しない構造的結果**なのでパラメータのCI無しでも崩れない。検証スクリプトは未昇格（scratchpad のみ、`発表準備.md` §4 タスク4で本採用予定）。

**学会発表の方針（2026-07-29決定）**: 数理生物学会 2026（**9/7–10, 口頭15分**）で発表。要旨は提出済み（タイトル「多種資源動態モデルを用いたレジーム別最大持続生産量(MSY)の推定」）。**発表の主軸は上記の構造定理**に置き、Catch-MSY結合の障害（和 vs 積、r の意味不整合）はそこへ至る診断として前段に配置する。スライド骨子（15枚＋予備6枚, 1枚1メッセージ・優先度と想定秒数つき）、想定質疑10問、9/7までの週次タスクは **`発表準備.md`** に集約。最優先タスクは文献確認（Volterra の原理／May et al. 1979／Legović et al. 2010 等）で、「既知の再発見」と見なされないための差分を言えるようにすること。
