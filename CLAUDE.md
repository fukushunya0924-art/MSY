...
**内点MSY非存在の構造定理（Phase 14, 2026-07-29／条件を2026-07-30に訂正）**: Phase 13 で経験的に観測された「上限張り付き・内部最大なし」は、グリッド探索の結果ではなく**解析的に導ける必然**であることを確認した。

- **成立条件（重要な訂正）**: 効いているのは「$A$ の対角0」**ではなく** $A$ の**ブロック反対角構造**（ギルド内相互作用がすべてゼロ＝被食者間competitionなし・捕食者間相互作用なし。対角0はその一部）。一般化LVの平衡は $B_{eq}(f)=B_{eq}(0)+A^{-1}f$ とfのアフィン関数で、感度は $\partial B_{eq,i}/\partial f_j=(A^{-1})_{ij}$（press perturbation）。よって必要なのは $\mathrm{diag}(A^{-1})=0$ であり、**$\mathrm{diag}(A)=0$ からこれは一般には従わない**（反例: $A=[[0,1,1],[1,0,1],[1,1,0]]$ の逆行列の対角は $-1/2$）。本モデルは $A=\begin{bmatrix}0&-L\\ C\circ L&0\end{bmatrix}$ なので $A^{-1}=\begin{bmatrix}0&(C\circ L)^{-1}\\ -L^{-1}&0\end{bmatrix}$ となり対角が消える。
- **帰結**: 平衡は2つの2×2線形系に分解され `L·y_eq = r_x − f_x` / `(C∘L)·x_eq = r_y + f_y`、すなわち **x_eq は f_x に、y_eq は f_y に一切依存しない**（自種の漁獲が自種の平衡量を変えない。古典的には Volterra 1926 の2種系の性質に対応する既知の事実であって、本研究の主張ではない）。ゆえに平衡収量 $Y_{eq}(f)=\sum_i f_i B_{eq,i}(f)=f\cdot B_{eq}(0)+f^{\mathsf T}A^{-1}f$ から $f_i^2$ の項が消えて **f について多重線形**となり、箱 $[0,f_{max}]^4$ 上の**最大は必ず頂点で達成される**（＝内点に厳密最大は存在しない）→ 報告値は探索上限 $f_{max}$ の人工物。
- **数値検証**（自由推定12変数, 既存 `sustainability.build_A_rho` を使用・コード無改変）: ∂x_eq/∂f_x = ∂y_eq/∂f_y = **厳密に0**（両レジーム）、各 f_j 方向の2階差分 = 4.3e−12(NLM)/1.4e−12(LM)＝機械精度、頂点16点の最大が内部ランダム20万点を上回る。
- **留保**: (a) 厳密な多重線形性は平衡収量の性質であり、実際の計算は有限T の時間平均収量＋持続性制約なので頂点性は厳密には保証されない（数値実験は同じ構造を示す）。(b) 本モデルでは正の内部平衡自体が存在しない（Phase 13）ので、この定理は「**仮に**正の内部平衡があっても内点MSYは無い」という上界の議論。(c) 自己制限なしLVの内部平衡は中立安定になりうるため平衡収量は形式的な参照量。
- **この結論は推定値に依存しない構造的結果**なのでパラメータのCI無しでも崩れない。検証スクリプトは未昇格（scratchpad のみ、`発表準備.md` §4 タスク4で本採用予定）。

**学会発表の方針（2026-07-29決定, 2026-07-30更新）**: 数理生物学会 2026（**9/7–10, 口頭15分**）で発表。要旨は提出済み（タイトル「多種資源動態モデルを用いたレジーム別最大持続生産量(MSY)の推定」）。**発表の主軸は上記の構造定理**に置き、Catch-MSY結合の障害（和 vs 積、r の意味不整合）はそこへ至る診断として前段に配置する。

> **Volterra の原理は「主張」として出さない方針（2026-07-30決定）**: 「自種の漁獲が自種の平衡量を変えない」は古典的既知事実であり、これを発表の売りにすると「既知の再発見」に見える。**発表で主張するのは「この モデルクラスでは平衡収量が f について多重線形 ⇒ 最適 f は常に探索箱の頂点 ⇒ 報告MSYは f_max の人工物」という計算枠組みへの帰結の方**。Volterra への言及はスライド上では脚注1行の出典表記に留め、質疑で問われたときのみ展開する。

スライド骨子（16枚＋予備7枚, 1枚1メッセージ・優先度と想定秒数つき／12分実走プランつき）、想定質疑13問、9/7までの週次タスクは **`発表準備.md`** に集約。同ファイル §5 に構造定理の完全な導出（2種の直感→A⁻¹のブロック公式→多重線形性→頂点最大、および教授質疑の先回り）を置いてある。

**次のステップ**:
0. **上限駆動・平衡非存在への対応（Phase 13の含意）**: 現行の密度依存なしLVでは内部MSYが定義できない。持続的な内部最大が要るなら種内競争項（α, 環境収容力）の再導入か、漁獲率上限を生物学的根拠で設定する必要。教授相談事項に追加。→ Phase 14 で「定義できない」ことの解析的根拠が確定。
