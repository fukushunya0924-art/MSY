...
|---|---|---|---|---|
| r_x1 | マイワシ | 0.899 [0.709,1.113] | [0.6,0.95]（標準ルール外の例外採用、要相談） | 暫定 |
| r_x2 | ウルメイワシ | 0.739 [0.642,0.824] | [0.01,0.4]（標準ルール通り） | ✅ 確定 |
| c1+d1 | ブリ | 0.402 [0.269,0.579] | [0.3,0.7]（標準ルール通り） | ✅ 確定 |
| c2+d2 | サワラ | 0.361 [0.251,0.478] | [0.01,0.4]（標準ルール通り） | ✅ 確定 |

> Catch-MSY の事前分布は原論文（Martell & Froese 2013）に合わせて修正済み（2026-07-12）:
> K上限倍率 60→100×max(catch)、初期枯渇度B0/Kのサンプル方法 log-uniform→uniform（`現行コード/catch_msy/catch_msy_core.py`）。
> 上表の数値はこの修正後の再計算結果。
