...
**次のステップ**:
1. 本番 `run_msy.py` フルランでPhase 5〜5bのMSY結果を再検証(局所解時代との差分確認)。
2. ウルメx2 NLMの当てはまりの悪さ(R²=−2.63)の原因切り分け(モデル vs 指標値ノイズ)。
3. 確定パラメータ(r_x1, r_x2, c1+d1, c2+d2)を固定 → 残りパラメータ推定 → MSY導出のパイプライン実装。
4. マイワシの r_x1=0.90(標準ルール外の高レンジ採用)の妥当性を教授と相談。
