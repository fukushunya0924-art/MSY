...
**共通漁獲率MSYは検討済み・不採用（Phase 13b, 2026-07-18）**: 「4種すべてに同じ f を一律適用して1次元最大化すれば内部ピークが立つのでは」という案を4ケース実測で検証した結果、**NLMでは内部ピークが立つが（マアジ f=0.373, マイワシ f=0.228）、LMでは両種構成とも f=0.95 に張り付いたまま**で、内部最大の有無がレジームで反転する。またこのピークは密度依存ではなく「全種同一f制約＋栄養段階カップリング」が生む見かけのもので、種の重み付け（等重み＝恣意的）に依存し生物学的不変量ではない。→ **主たるMSY定義には採用せず**、`uniform-effort yield curve（一律漁獲努力の収量応答曲線）` として `MSY.md` §8 に補助解析として記載（Phase 13 の主張を補強する傍証）。詳細 `docs/research_log.md` Phase 13b。
