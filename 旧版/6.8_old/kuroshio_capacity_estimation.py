...
def estimate_full_free(years, x1_n, x2_n, y1_n, y2_n, fx1, fx2, fy1, fy2, mean_x1, mean_x2, mean_y1, mean_y2, regime_name):
    ...
    def ode_fun(t, state, params):
        ...
        # 16個の正規化パラメータを受け取る
        r_x1, r_x2, r_y1, r_y2, L11, L12, L21, L22, C1, D1, C2, D2, A_x1, A_x2, A_y1, A_y2 = params
        
        x1_v = max(1e-5, x1_v); x2_v = max(1e-5, x2_v); y1_v = max(1e-5, y1_v); y2_v = max(1e-5, y2_v)
        
        # 16変数フルオープンモデルの微分方程式 (正規化空間)
        dx1dt = (r_x1 - A_x1 * x1_v - fx1_i(t)) * x1_v - L11 * x1_v * y1_v - L12 * x1_v * y2_v
        dx2dt = (r_x2 - A_x2 * x2_v - fx2_i(t)) * x2_v - L21 * x2_v * y1_v - L22 * x2_v * y2_v
        dy1dt = (-r_y1 - A_y1 * y1_v - fy1_i(t)) * y1_v + C1 * L11 * x1_v * y1_v + D1 * L21 * x2_v * y1_v
        dy2dt = (-r_y2 - A_y2 * y2_v - fy2_i(t)) * y2_v + C2 * L12 * x1_v * y2_v + D2 * L22 * x2_v * y2_v
    ...
    # 初期推測値 (16変数, 正規化空間)
    guess = [1.5, 1.5, 0.3, 0.4, 0.1, 0.1, 0.1, 0.1, 0.15, 0.15, 0.15, 0.15, 0.01, 0.01, 0.01, 0.01]
    
    # 境界条件 (下限 / 上限)
    lower = [0.1,  0.1,  0.01, 0.01, 1e-4, 1e-4, 1e-4, 1e-4, 0.001, 0.001, 0.001, 0.001, 1e-6, 1e-6, 1e-6, 1e-6]
    upper = [5.0,  5.0,  2.0,  2.0,  5.0,  5.0,  5.0,  5.0,  10.0,  10.0,  10.0,  10.0,  1.0,  1.0,  1.0,  1.0]
    
    print(f"--- 16変数全自由推定開始 (正規化): {regime_name} ---")
    ...
    r_x1_est, r_x2_est, r_y1_est, r_y2_est, L11_est, L12_est, L21_est, L22_est, C1_est, D1_est, C2_est, D2_est, A_x1_est, A_x2_est, A_y1_est, A_y2_est = res.x
    ...
    alpha_x1_est = A_x1_est / mean_x1
    alpha_x2_est = A_x2_est / mean_x2
    alpha_y1_est = A_y1_est / mean_y1
    alpha_y2_est = A_y2_est / mean_y2
    
    params_absolute = [
        r_x1_est, r_x2_est, r_y1_est, r_y2_est,
        l11_est, l12_est, l21_est, l22_est,
        c1_est, d1_est, c2_est, d2_est,
        alpha_x1_est, alpha_x2_est, alpha_y1_est, alpha_y2_est
    ]
    
    return params_absolute, traj_absolute

# 各期間での自由推定の実行 (正規化されたデータを使用)
...
# ==========================================
print("\n" + "="*50)
print("【16変数全自由・対数誤差推定 結果比較（元スケール換算値）】")
print("="*50)
p_names = [
    "r_x1 (マイワシ内的増加率)          ", "r_x2 (カタクチイワシ内的増加率)    ", 
    "r_y1 (ヤリイカの自然死亡率)        ", "r_y2 (スルメイカの自然死亡率)      ", 
    ...
    "alpha_x1 (マイワシ種内競争)          ", "alpha_x2 (カタクチイワシ種内競争)  ",
    "alpha_y1 (ヤリイカ種内競争)          ", "alpha_y2 (スルメイカ種内競争)      "
]
...
for idx in range(4):
    ...
    axes[idx, 0].set_title(f"NLM期 (16変数・対数誤差): {species_labels[idx]}")
    ...
    axes[idx, 1].set_title(f"LM期 (16変数・対数誤差): {species_labels[idx]}")
    axes[idx, 1].set_ylabel('資源量（千トン）')
    axes[idx, 1].grid(True, linestyle='--')
    axes[idx, 1].legend()

plt.tight_layout()
output_img_path = os.path.join(os.path.dirname(__file__), 'kuroshio_full_free_fitting.png')
