...
def estimate_no_capacity_full(years, x1, x2, y1, y2, fx1, fx2, fy1, fy2, regime_name):
    ...
    def ode_fun(t, state, params):
        ...
        # alpha (種内競争) を排除し、r_y, c, d を推定する 12変数
        r_x1, r_x2, r_y1, r_y2, l11, l12, l21, l22, c1, d1, c2, d2 = params
    ...
    # 初期推測値 (12変数)
    guess = [1.5, 1.5, 0.3, 0.4, 0.001, 0.001, 0.001, 0.001, 0.15, 0.15, 0.15, 0.15]
    
    # 境界条件 (下限 / 上限)
    lower = [0.1,  0.1,  0.01, 0.01, 1e-6, 1e-6, 1e-6, 1e-6, 0.01, 0.01, 0.01, 0.01]
    upper = [5.0,  5.0,  2.0,  2.0,  0.05, 0.05, 0.05, 0.05, 2.0,  2.0,  2.0,  2.0]
    
    print(f"--- 環境収容力なし (全パラメータ自由推定) 推定開始: {regime_name} ---")
...
# 推定の実行
params_nlm, traj_nlm = estimate_no_capacity_full(
...
params_lm, traj_lm = estimate_no_capacity_full(
...
for idx in range(4):
    # NLMプロット
    axes[idx, 0].plot(years_nlm, traj_nlm[idx], 'b-', linewidth=2.5, label='自由推定モデル軌道')
    axes[idx, 0].plot(years_nlm, [x1_all[idx_nlm], x2_all[idx_nlm], y1_all[idx_nlm], y2_all[idx_nlm]][idx], 'ro', alpha=0.7, label='実データ')
    axes[idx, 0].set_title(f"NLM期 (収容力なし・全自由): {species_labels[idx]}")
    axes[idx, 0].set_ylabel('資源量（千トン）')
    axes[idx, 0].grid(True, linestyle='--')
    axes[idx, 0].legend()
    
    # LMプロット
    axes[idx, 1].plot(years_lm, traj_lm[idx], 'g-', linewidth=2.5, label='自由推定モデル軌道')
    axes[idx, 1].plot(years_lm, [x1_all[idx_lm], x2_all[idx_lm], y1_all[idx_lm], y2_all[idx_lm]][idx], 'ro', alpha=0.7, label='実データ')
    axes[idx, 1].set_title(f"LM期 (収容力なし・全自由): {species_labels[idx]}")
    axes[idx, 1].set_ylabel('資源量（千トン）')
    axes[idx, 1].grid(True, linestyle='--')
    axes[idx, 1].legend()

plt.tight_layout()
plt.savefig('kuroshio_no_capacity_full_fitting.png', dpi=150)
plt.close()
print("\n対比グラフが 'kuroshio_no_capacity_full_fitting.png' としてカレントディレクトリに保存されました。")
