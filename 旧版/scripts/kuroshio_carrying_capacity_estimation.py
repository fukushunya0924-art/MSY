...
df_mackerel = pd.read_csv('マサバ時系列データ_資源量・漁獲量・漁獲係数 - マサハバ時系列データ_資源量・漁獲量・漁獲係数.csv')
...
df_merged = df_squid[['年', '資源量（千トン）', '漁獲量（千トン）', '黒潮大蛇行の有無']].merge(
    ...
    df_mackerel_renamed = df_mackerel[['年', '資源量（万トン）', '漁獲量（万トン）']], on='年', suffixes=('_sardine', '_mackerel')
)
...
def estimate_with_capacity(years, x1, x2, y1, y2, fx1, fx2, fy1, fy2, regime_name):
    ...
    def ode_fun(t, state, params):
        ...
        # 10個のパラメータを受け取る (増加率2個 + 捕食効率4個 + 種内競争4個)
        r_x1, r_x2, l11, l12, l21, l22, alpha_x1, alpha_x2, alpha_y1, alpha_y2 = params
        
        x1_v = max(1e-5, x1_v); x2_v = max(1e-5, x2_v); y1_v = max(1e-5, y1_v); y2_v = max(1e-5, y2_v)
        
        # 環境収容力（種内競争項: -alpha * N^2）を組み込んだ微分方程式
    ...
    # 初期推測値: [r_x1, r_x2, l11, l12, l21, l22, alpha_x1, alpha_x2, alpha_y1, alpha_y2]
    guess = [1.5, 1.5, 0.001, 0.001, 0.001, 0.001, 0.0001, 0.0001, 0.0001, 0.0001]
    
    # 境界条件の設定
    lower = [0.1,  0.1,  1e-6, 1e-6, 1e-6, 1e-6, 1e-7, 1e-7, 1e-7, 1e-7]
    upper = [5.0,  5.0,  0.05, 0.05, 0.05, 0.05, 0.01, 0.01, 0.01, 0.01]
    
    print(f"--- 環境収容力モデル最適化開始: {regime_name} ---")
...
for idx in range(4):
    # NLMプロット
    axes[idx, 0].plot(years_nlm, traj_nlm[idx], 'b-', linewidth=2.5, label='種内競争モデル軌道')
    ...
    # LMプロット
    axes[idx, 1].plot(years_lm, traj_lm[idx], 'g-', linewidth=2.5, label='種内競争モデル軌道')
...
plt.tight_layout()
plt.savefig('kuroshio_capacity_fitting.png', dpi=150)
plt.close()
print("\nフィッティング画像が 'kuroshio_capacity_fitting.png' として保存されました。")