"""
マイワシ+カタクチイワシ(x) / ブリ+スケトウダラ(y) で capacity_ry(12変数)推定。

使い方:
  python3 run_estimation.py              # capacity_ry (デフォルト)
  python3 run_estimation.py capacity     # capacity (10変数)
出力:
  - コンソールに R²/NRMSE・パラメータ一覧
  - 現行コード/2026-06-23/fit_<model>.png
"""
import os
import sys
import numpy as np
...
def main():
    ...
    print(f"=== マイワシ+カタクチイワシ(x) / ブリ+スケトウダラ(y)  モデル: {model} ===")

    df = load_clean_dataframe()
    series = get_series(df)

    print(f"\nデータ確認: {int(series['years'].min())}–{int(series['years'].max())} 年  ({len(series['years'])}点)")
    for k, lab in zip(KEYS, SPECIES_LABELS):
        print(f"  {lab:25s}: 平均 {series[k].mean():.1f} 千トン")
    ...
    plot(results, model)
    out_path = os.path.join(_here, f"fit_{model}.png")
    print(f"\nグラフを {out_path} に保存しました。")
...
def plot(results, model):
    ...
    fig.suptitle(f"マイワシ+カタクチ(x) / ブリ+スケトウダラ(y) — {model}", fontsize=14, y=1.005)
    plt.tight_layout()
    out = os.path.join(_here, f"fit_{model}.png")
