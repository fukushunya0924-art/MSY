"""
1位の組み合わせ（ヤリイカ+スルメイカ / ブリ+サワラ）で推定を走らせる。

親フォルダ improved/ の model.py（マルチスタート+正則化+LSODA+R²指標）を再利用。

使い方:
  python3 run_rank1.py            # full(16変数)
  python3 run_rank1.py capacity   # capacity(10変数)
出力:
  - コンソールに R²/NRMSE 比較・境界張り付き診断
  - improved/rank1_squid_largefish/fit_rank1_<model>.png
"""
import os
import sys
import numpy as np
...
# ローカル（この組み合わせ用）の data_loader を最優先で読むため先頭に。
# model.py は親フォルダ improved/ から再利用するので末尾に追加（ローカルを上書きしない）。
...
def main():
    ...
    print(f"=== 1位組み合わせ: ヤリイカ+スルメイカ(x) / ブリ+サワラ(y) ===  モデル: {model}")
    ...
    reg = {"full": 0.02, "capacity_ry": 0.01, "capacity": 0.005}.get(model, 0.01)
    ...
    plot(results, model)
    print(f"\nグラフを improved/rank1_squid_largefish/fit_rank1_{model}.png に保存しました。")
...
def plot(results, model):
    ...
    fig.suptitle(f"1位組み合わせ ヤリイカ+スルメイカ/ブリ+サワラ — {model}", fontsize=14, y=1.005)
    plt.tight_layout()
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"fit_rank1_{model}.png")
