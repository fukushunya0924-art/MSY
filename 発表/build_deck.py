...
本編16枚＋予備8枚＋マイワシ版5枚。白背景・色数を絞る・全ページ番号つき。
...
textbox(s, ML, 5.02, CW - 1.30, 0.30,
        "左＝制約なしの上限感度、右＝制約つきグリッド探索の f*。"
        "どちらも漁獲率の刻みは0.05に固定（制約MSY 527.7／254.9 千トン/年）。",
        size=11.5, color=GRAY)
b = box(s, ML, 5.42, CW, 1.35, fill=TINT2, line=ORANGE)
textbox(s, ML + 0.22, 5.56, CW - 0.44, 1.10,
        ["制約なしでは、探索の上限を 0.25 → 1.25 と上げると答えも 207 → 1094 千トンと上がり続ける。",
         "資源が減りすぎない条件をつけても、サワラは両時期とも上限 0.95 に張り付いたまま。",
...
s = new_slide("［マイワシ版・8枚目］どちらの時期も、観測をよく再現できた")
figure(s, "05m_当てはまり_マイワシ", 1.17, 1.32)
textbox(s, ML, 5.95, CW, 0.30,
        "全体の平均（大蛇行なし／大蛇行）：　12個 0.145／0.079　　"
        "10個 0.306／0.115　　8個 0.408／0.122", size=14, color=GRAY)
b = box(s, ML, 6.32, CW - 0.8, 0.56, fill=TINT2, line=ORANGE)
textbox(s, ML + 0.22, 6.44, CW - 1.24, 0.36,
        "餌の魚をマイワシにしても、固定を増やすほど当てはまりが悪くなる順序は変わらない。",
        size=17, bold=True, color=ORANGE)

# ============================================== 26 マイワシ版の借りる値（9枚目）
s = new_slide("［マイワシ版・9枚目］借りられるのは「増える速さ」まで")
lead(s, "「和を固定すると、かけ算を保つために食べられる強さが歪む」ことは、"
        "もともとマイワシ版で見つけた現象。")
...
box(s, ML, 4.02, CW, 1.24, fill=SHADE, line=None)
textbox(s, ML + 0.24, 4.18, CW - 0.48, 1.00,
        ["マイワシ版で和を固定すると、ウルメ→ブリ の λ が 0.224 → 0.945 まで大きくなる"
         "（大蛇行なし、12個の自由推定 → 8個の固定）。",
         "マアジ版でも同じことが起きる（0.070 → 0.869）。歪む量は種構成で変わるが、"
         "向きは同じ。"],
        size=15.5, space_after=7)

b = box(s, ML, 5.42, CW, 0.62, fill=TINT2, line=ORANGE)
textbox(s, ML + 0.22, 5.56, CW - 0.44, 0.4,
        "借りられるのは「増える速さ」まで、という結論は種構成に左右されない。",
        size=17, bold=True, color=ORANGE)
note(s, "λ は平均を1にそろえた空間での値（9枚目と同じ定義）。"
        "9枚目に載せた 0.224 という値は、もともとこのマイワシ版で測ったもの。",
     y=6.25, size=14, height=0.62)

# ============================================== 27 マイワシ版の r の意味（10枚目）
s = new_slide("［マイワシ版・10枚目］同じ記号 r でも、中身が違う")
figure(s, "07m_増える速さと漁獲率_マイワシ", ML, 1.45)
textbox(s, 7.72, 1.42, 4.98, 4.2,
        [("マイワシでは逆転しない", {"size": 17, "bold": True, "color": NAVY}),
         "借りた r 0.940 は、実際にとっている割合 0.300 より大きい。"
         "マアジで起きた「食べられる前から毎年減る」という崩れ方はしない。",
         ("それでも、借りた値は合っていない", {"size": 17, "bold": True,
                                              "color": ORANGE}),
         "多種の式が自分で選ぶ増える速さは 0.624（大蛇行なし）・3.00（大蛇行）で、"
         "借りた 0.940 とは時期ごとに食い違う。",
         "しかもその 0.940 は、1種ずつの解析が通常の条件では解けず、"
         "資源が高水準という例外の設定でしか出てこない値。"],
        size=15.5, space_after=9)
b = box(s, ML, 5.75, CW, 0.62, fill=TINT2, line=ORANGE)
textbox(s, ML + 0.22, 5.89, CW - 0.44, 0.4,
        "崩れ方は種で違うが、原因は同じ。1種ずつの解析の r は、そのままでは多種の式に入らない。",
        size=17, bold=True, color=ORANGE)
note(s, "マアジは 借りた 0.228 ＜ とっている割合 0.431 で逆転が起きた（10枚目）。"
        "マイワシは逆転しない代わりに、借りる値そのものが例外の設定に頼っている。",
     y=6.55, size=14, height=0.62)

# ============================================== 28 マイワシ版の端張り付き（13枚目）
# f* の値は 現行コード/msy/outputs/マイワシ版/自由推定/
#   msy_グリッド散布_持続可能制約_マイワシ_ウルメイワシ_ブリ_サワラ_capacity_ry.png
# の星（探索の最良点）そのまま。制約MSY は 695.91 ／ 10986.28 千トン/年（刻み0.05の格子）。
# 上限感度は outputs/sustainability_sensitivity_マイワシ_{NLM,LM}_step0.05.csv の
#   upper_bound_unconstrained 行の合計（全条件で刻み0.05に固定した版）。
s = new_slide("［マイワシ版・13枚目］制約つきの最適でも、端に張り付く")
figure(s, "09m_端に張り付く_マイワシ", ML, 1.45)
textbox(s, 7.72, 1.42, 4.98, 4.25,
        [("端に張り付く種が残る", {"size": 17, "bold": True, "color": ORANGE}),
         "大蛇行なしではサワラが上限 0.95、大蛇行ではマイワシが上限 0.95。"
         "資源が減りすぎない条件をつけたうえでの最適が、これ。",
         ("制約つきでとれる量", {"size": 17, "bold": True, "color": NAVY}),
         "大蛇行なし 695.9 ／ 大蛇行 10986.3 千トン/年。大蛇行が1桁以上大きいのは、"
         "マイワシの資源量が桁違いで、その上限まで取る解が選ばれるため。",
         ("制約なしの上限感度も同じ", {"size": 17, "bold": True, "color": NAVY}),
         "探索の上限を 0.25 → 1.25 と上げると、大蛇行なしは 366 → 908、"
         "大蛇行は 1266 → 35573 千トンと上がり続ける。"],
        size=15, space_after=8)
b = box(s, ML, 5.75, CW, 0.62, fill=TINT2, line=ORANGE)
textbox(s, ML + 0.22, 5.89, CW - 0.44, 0.4,
        "餌の魚を替えても、最大は範囲の内側になく、探索の上限が答えを決めている。",
        size=17, bold=True, color=ORANGE)
note(s, "縦軸は最適とされた漁獲率（制約つき）。刻み0.05の格子・持続可能制約つき"
        "（最終年の資源量が初年の9割以上）、12個の自由推定。"
        "マアジ版の同じ図は13枚目（制約MSY 527.7／254.9 千トン/年）。",
     y=6.55, size=14, height=0.62)

# ============================================== 29 マイワシ版の係数一覧（表）
# 3つの版（12個の自由推定 / 10個 / 8個）の推定値を並べる。数値の出どころは
#   12個: docs/research_log.md Phase 8 の絶対値（means で正規化空間へ戻したもの）
#   10個: msy/estimates_マイワシ_capacity_ry_constrained.pkl
#    8個: msy/estimates_マイワシ_capacity_ry_constrained_v1_8var.pkl
FIX = {"color": GRAY}          # 1種ずつの解析から借りて固定した値
BIG = {"color": ORANGE, "bold": True}   # 固定のしわ寄せで極端になった値

s = new_slide("［マイワシ版・参考］3つの版で推定された係数")
textbox(s, ML, 1.28, CW, 0.52,
        ["値は平均を1にそろえた空間のもの（9枚目の λ と同じ定義）。"
         "10個＝餌2種の「増える速さ」を固定、8個＝さらに「変換の割合の和」も固定。",
         "灰色＝借りて固定した値（8個は元の単位で c₁+d₁=0.395、c₂+d₂=0.26 も固定）。"
         "橙＝しわ寄せで極端になった値。≈0 は 0.001 未満。"],
        size=12, color=GRAY, space_after=2)

NUMW = 1.44
X0 = ML + 3.45
textbox(s, X0, 1.80, NUMW * 3, 0.28, "大蛇行なし（2006〜2016年）", size=14,
        bold=True, color=NAVY, align=PP_ALIGN.CENTER)
textbox(s, X0 + NUMW * 3, 1.80, NUMW * 3, 0.28, "大蛇行（2017〜2024年）",
        size=14, bold=True, color=ORANGE, align=PP_ALIGN.CENTER)

table(s, ML, 2.10, CW,
      [["係数", "12個（自由）", "10個", "8個", "12個（自由）", "10個", "8個"],
       ["r_x1　マイワシが増える速さ",
        "0.624", ("0.940", FIX), ("0.940", FIX),
        "3.004", ("0.940", FIX), ("0.940", FIX)],
       ["r_x2　ウルメが増える速さ",
        "0.472", ("0.739", FIX), ("0.739", FIX),
        "0.651", ("0.739", FIX), ("0.739", FIX)],
       ["r_y1　ブリが自然に減る速さ",
        "0.032", "0.030", "0.012", "0.094", "0.848", "0.011"],
       ["r_y2　サワラが自然に減る速さ",
        "0.011", "0.011", "0.010", "0.069", "0.063", "0.163"],
       ["λ11　食べられる強さ：マイワシ→ブリ",
        "0.035", "≈0", "0.692", "2.077", "0.595", "0.619"],
       ["λ12　食べられる強さ：マイワシ→サワラ",
        "0.055", "0.563", "0.005", "0.563", "≈0", "≈0"],
       ["λ21　食べられる強さ：ウルメ→ブリ",
        "0.224", "0.560", ("0.945", BIG), "0.138", "0.379", "0.642"],
       ["λ22　食べられる強さ：ウルメ→サワラ",
        "0.175", "0.225", "0.028", "0.477", "0.333", "0.102"],
       ["C1　変換の割合：マイワシ→ブリ",
        "0.485", ("6.012", BIG), "0.140", "0.101", "0.933", "0.387"],
       ["D1　変換の割合：ウルメ→ブリ",
        "2.119", "0.766", "0.272", "1.522", "1.654", "0.203"],
       ["C2　変換の割合：マイワシ→サワラ",
        "0.191", "0.105", "0.728", "0.030", "0.004", ("64.2", BIG)],
       ["D2　変換の割合：ウルメ→サワラ",
        "2.544", "1.619", ("12.8", BIG), "0.773", "1.113", "4.476"],
       [("平均のずれ（NRMSE, 4種平均）", {"bold": True}),
        ("0.145", {"bold": True}), ("0.306", {"bold": True}),
        ("0.408", {"bold": True}), ("0.079", {"bold": True}),
        ("0.115", {"bold": True}), ("0.122", {"bold": True})]],
      col_w=[3.45] + [NUMW] * 6, size=12, row_h=0.30, head_h=0.36)

textbox(s, ML, 6.44, CW - 1.30, 0.64,
        "固定を増やすほど、食べられる強さと変換の割合が極端な値へ動く"
        "（ウルメ→ブリ 0.224→0.945、マイワシ→サワラの変換 0.030→64.2）。"
        "当てはまりの悪化は、その歪みの結果。",
