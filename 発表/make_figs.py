...
from model import make_ode, simulate, compute_metrics
...
# -------------------------------------------------- マイワシ版（末尾の3枚用）
# 餌の魚 x1 を置き換える前の種構成。図は 05m / 07m として別に作る。
import data_loader_iwashi as dli
from run_sustainability_diagnostics_iwashi import reconstruct_iwashi_estimates

JP_I = ["マイワシ", "ウルメイワシ", "ブリ", "サワラ"]
series_i = dli.get_series(dli.load_clean_dataframe())
_nlm_i, _lm_i = dli.regime_masks(series_i)
SL_I = {"NLM": dli.slice_series(series_i, _nlm_i),
        "LM": dli.slice_series(series_i, _lm_i)}

# 12変数の自由推定は pkl がなく、docs/research_log.md Phase 8 の絶対値から
# 逆変換して復元する（再推定なし・round-trip 検証つき）。
_EST_I_FREE = reconstruct_iwashi_estimates([("NLM", SL_I["NLM"]), ("LM", SL_I["LM"])])
VERSIONS_I = [
    ("12個（自由）", _EST_I_FREE, NAVY, (0, ()), 2.4),
    ("10個", _est("estimates_マイワシ_capacity_ry_constrained.pkl"),
     "#5E8FB5", (0, (5, 2.4)), 2.0),
    ("8個", _est("estimates_マイワシ_capacity_ry_constrained_v1_8var.pkl"),
     "#7E8B96", (0, (1.2, 2.0)), 2.1),
]
...
def nrmse_by_species(sl, res):
    """魚種ごとの「ずれ」。推定結果に入っていなければ観測年で計算し直す。

    復元した12変数版（マイワシ）は魚種ごとの値を持たないため。10変数・8変数の
    pkl では、計算し直した値が保存値と小数3桁まで一致することを確認済み。
    """
    m = res.get("metrics", {})
    if "x1" in m:
        return [m[KEYS[i]]["NRMSE"] for i in range(4)]
    years = sl["years"]
    t_rel = (years - years.min()).astype(float)
    means = res["means"]
    init = [sl[KEYS[i]][0] / means[i] for i in range(4)]
    f_interp = [interp1d(t_rel, sl["f" + KEYS[i]], kind="linear",
                         fill_value="extrapolate") for i in range(4)]
    yn = simulate(res["params_norm"], make_ode(*f_interp), t_rel, init)
    traj = np.vstack([yn[i] * means[i] for i in range(4)])
    calc = compute_metrics(traj, [sl[KEYS[i]] for i in range(4)])
    return [calc[KEYS[i]]["NRMSE"] for i in range(4)]
...
# ============================================================ 05 当てはまり
def fig_fit(name="05_当てはまり", sl_map=None, versions=None, jp=None):
    sl_map = SL if sl_map is None else sl_map
    versions = VERSIONS if versions is None else versions
    jp = JP if jp is None else jp
    W, H = 11.0, 4.55
    ...
    for row, reg in enumerate(["NLM", "LM"]):
        ...
        nrm = [nrmse_by_species(sl, est[reg]) for _, est, _, _, _ in versions]
        for i in range(4):
            ax = axes[row, i]
            ...
            for k, (_, _, col, _, _) in enumerate(versions):
                ax_put(name, ax, 0.05 + k * 0.33, 0.925, f"{nrm[k][i]:.3f}",
...
# ==================================================== 07m r と f（マイワシ版）
def fig_r_vs_f_iwashi():
    name, W, H = "07m_増える速さと漁獲率_マイワシ", 6.75, 3.15
    fig, ax = plt.subplots(figsize=(W, H))
    fig.subplots_adjust(left=0.105, right=0.985, top=0.955, bottom=0.155)
    r_fix = [0.940, 0.739]
    f_obs = [float(series_i["fx1"].mean()), float(series_i["fx2"].mean())]
    x = np.arange(2); w = 0.36
    ax.bar(x - w / 2, r_fix, w, color=NAVY)
    ax.bar(x + w / 2, f_obs, w, color=ORANGE)
    for xi, v in zip(x - w / 2, r_fix):
    ...
    ax.tick_params(labelsize=10)
    ax.grid(True, axis="y", ls="--", alpha=0.35)
    # マアジ版のような「逆転」の囲みは要らない（逆転しないことが要点）
    ax.add_patch(Rectangle((0.38, 1.19), 0.07, 0.055, color=NAVY))
    ax.add_patch(Rectangle((0.38, 1.06), 0.07, 0.055, color=ORANGE))
    ax_put(name, ax, 0.44, 0.935, "1種ずつの解析から借りた「増える速さ」",
           size=10.5, ha="l", w=3.3)
    ax_put(name, ax, 0.44, 0.835, "実際にとっている割合（1994〜2024年）",
           size=10.5, ha="l", w=3.3)
    for xi, t in zip(x, ["マイワシ", "ウルメイワシ"]):
        ax_put(name, ax, (xi + 0.5) / 2, -0.055, t, size=12.5, ha="c", va="t", w=1.6)
    ax_put(name, ax, 0.25, 0.865, "マイワシでは逆転しない", size=12, color=NAVY,
           ha="c", w=2.1)
    put(name, 0.012, 0.985, "縦軸：1年あたりの割合", size=11.5, color=GRAY,
        ha="l", va="t", w=2.2)
    save(fig, name, W, H)
...
def fig_upper():
    ...
    # outputs/sustainability_sensitivity_{NLM,LM}_step0.05.csv の
    # upper_bound_unconstrained 行の合計（全条件で刻み0.05に固定した版）
    nlm = [206.7, 381.6, 607.6, 802.0, 1093.7]
    lm = [140.6, 203.8, 246.0, 284.5, 351.9]
    ax = axes[0]
    ...
    title_of(name, ax, "制約なし：上限を上げるほど、答えも上がり続ける", size=13.5)
    xlabel_of(name, ax, "探索で許した漁獲率の上限", dy=0.085, size=12)
    put(name, 0.004, 0.030, "縦軸：とれる量（千トン）", size=11,
        color=GRAY, ha="l", va="c", w=2.0)
    ax_put(name, ax, 0.165, 0.930, "大蛇行なし", size=11, ha="l", w=1.1)
    ax_put(name, ax, 0.165, 0.828, "大蛇行", size=11, ha="l", w=1.1)

    # 右：制約つきグリッド探索の最適 f*（run_msy.py の 20^4 グリッド＝刻み0.05、自由推定）
    #   NLM 制約MSY=527.66 / LM 制約MSY=254.86 千トン/年。
    #   0 と 0.95 が探索範囲の端。出典は outputs/マアジ版/自由推定/
    #   msy_グリッド散布_持続可能制約_マアジ_..._capacity_ry.png の星。
    ax = axes[1]
    f_nlm = [0.40, 0.10, 0.90, 0.95]
    f_lm = [0.45, 0.05, 0.55, 0.95]
    x = np.arange(4); w = 0.36
    ax.bar(x - w / 2, np.maximum(f_nlm, 0.006), w, color=NAVY)
    ax.bar(x + w / 2, np.maximum(f_lm, 0.006), w, color=ORANGE)
    ax.axhline(0.95, color=GRAY, ls="--", lw=1.8)
    for xi, v in zip(x - w / 2, f_nlm):
        ax.text(xi, v + 0.035, f"{v:.3f}", ha="center", fontsize=10.5,
                color=NAVY)
    for xi, v in zip(x + w / 2, f_lm):
        ax.text(xi, v + 0.035, "0" if v == 0 else f"{v:.3f}", ha="center",
    ...
    ax.tick_params(labelsize=10)
    ax.grid(True, axis="y", ls="--", alpha=0.35)
    ax.add_patch(Rectangle((-0.40, 1.200), 0.16, 0.05, color=NAVY))
    ax.add_patch(Rectangle((-0.40, 1.080), 0.16, 0.05, color=ORANGE))
    title_of(name, ax, "制約つきの最適でも、端に張り付く種が残る", size=13.5)
    ax_put(name, ax, 0.100, 0.942, "大蛇行なし", size=11, ha="l", w=1.2)
    ax_put(name, ax, 0.100, 0.850, "大蛇行", size=11, ha="l", w=1.2)
    ax_put(name, ax, 0.214, 0.758, "上限 0.95", size=11, color=GRAY, ha="l",
           w=1.3)
    for i, t in enumerate(JP):
        ax_put(name, ax, (i + 0.6) / 4.2, -0.055, t, size=12, ha="c", va="t",
               w=1.6)
    put(name, 0.996, 0.030, "縦軸：最適とされた漁獲率（制約つき）", size=11,
        color=GRAY, ha="r", va="c", w=3.2)
    save(fig, name, W, H)


# ================================================== 09m 端に張り付く（マイワシ）
def fig_upper_iwashi():
    """マイワシ版の f*（持続可能制約つきグリッド探索）。

    数値は 現行コード/msy/outputs/マイワシ版/自由推定/
    msy_グリッド散布_持続可能制約_マイワシ_ウルメイワシ_ブリ_サワラ_capacity_ry.png
    の星（探索の最良点）そのまま。制約MSY は 大蛇行なし 695.91 ／ 大蛇行 10986.28
    千トン/年。0.95 が探索範囲の端。格子は 20^4＝刻み0.05。
    """
    name, W, H = "09m_端に張り付く_マイワシ", 6.9, 3.30
    fig, ax = plt.subplots(figsize=(W, H))
    fig.subplots_adjust(left=0.095, right=0.985, top=0.955, bottom=0.155)
    f_nlm = [0.10, 0.15, 0.85, 0.95]
    f_lm = [0.95, 0.15, 0.75, 0.50]
    x = np.arange(4); w = 0.36
    ax.bar(x - w / 2, f_nlm, w, color=NAVY)
    ax.bar(x + w / 2, f_lm, w, color=ORANGE)
    ax.axhline(0.95, color=GRAY, ls="--", lw=1.8)
    for xi, v in zip(x - w / 2, f_nlm):
        ax.text(xi, v + 0.035, f"{v:.3f}", ha="center", fontsize=10.5,
                color=NAVY)
    ...
    ax.tick_params(labelsize=10)
    ax.grid(True, axis="y", ls="--", alpha=0.35)
    ax.add_patch(Rectangle((-0.42, 1.205), 0.16, 0.05, color=NAVY))
    ax.add_patch(Rectangle((-0.42, 1.085), 0.16, 0.05, color=ORANGE))
    ax_put(name, ax, 0.105, 0.945, "大蛇行なし", size=11, ha="l", w=1.2)
    ax_put(name, ax, 0.105, 0.853, "大蛇行", size=11, ha="l", w=1.2)
    # 「上限 0.95」は、棒の低いウルメイワシの上（点線のすぐ上）が空いている
    ax_put(name, ax, 0.380, 0.762, "上限 0.95", size=11, color=GRAY, ha="c",
           w=1.3)
    for i, t in enumerate(JP_I):
        ax_put(name, ax, (i + 0.6) / 4.2, -0.055, t, size=12, ha="c", va="t",
               w=1.6)
    # 縦軸の説明は、図が横に狭く置き場がないのでスライド側の注記に回す
    save(fig, name, W, H)
...
if __name__ == "__main__":
    ...
    fig_vertex()
    # 末尾のマイワシ版4枚ぶん
    fig_fit("05m_当てはまり_マイワシ", SL_I, VERSIONS_I, JP_I)
    fig_r_vs_f_iwashi()
    fig_upper_iwashi()
